...
function buildCollaborationGitHookBlock(hookName: "pre-commit" | "pre-push"): string {
  ...
  return [
    ...
    "  }",
    '  if [ "${SNIPARA_COLLABORATION_GUARD_ACK_REVIEW_ONLY:-0}" = "1" ]; then',
    "    snipara_companion_guard --ack-review-only",
    "    snipara_guard_status=$?",
    '    if [ "$snipara_guard_status" -eq 0 ]; then',
    "      snipara_companion_guard",
    "      snipara_guard_status=$?",
    "    fi",
    '    if [ "$snipara_guard_status" -ne 0 ]; then',
    '      exit "$snipara_guard_status"',
    "    fi",
    "  else",
    "    snipara_companion_guard",
    "  fi",
