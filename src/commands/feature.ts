...
import {
  ...
  type GeneratedWorkflowTaskDocument,
...
export function buildFeaturePlanMarkdown(
  ...
  const steps = plan.steps
    .map((step, index) => {
      ...
      const verify = step.verify?.length ? `\n- Verify: ${step.verify.join("; ")}` : "";
      const dependency = step.depends_on?.length
        ? `\n- Depends on: ${step.depends_on.join(", ")}`
        : "";
      const parallel = step.parallel_group ? `\n- Parallel group: ${step.parallel_group}` : "";
      const nested = (step.tasks ?? [])
        .map((task) => `\n  - Task ${task.id}: ${quoteMarkdown(task.query)}`)
        .join("");
      return `### ${index + 1}. ${step.title}\n\n- Scope: ${quoteMarkdown(step.query)}${acceptance}${files}${verify}${dependency}${parallel}${nested}`;
    })
    .join("\n\n");
  return `# Technical Plan: ${manifest.slug}
...
export function buildFeatureTasksMarkdown(
  ...
  const tasks = plan.steps
    .map((step, index) => {
      const stepTasks: GeneratedWorkflowTaskDocument[] = step.tasks?.length
        ? step.tasks
        : [
            {
              id: step.id,
              ...
              verify: step.verify,
              commit: step.commit,
              depends_on: step.depends_on?.length
                ? step.depends_on
                : index > 0
              ...
              recovery_task_id: step.recovery_task_id,
            },
          ];
      return stepTasks
        .map((task, taskIndex) => {
          ...
          const acceptance = task.acceptance
            ? `\n  - Done when: ${quoteMarkdown(task.acceptance)}`
            : "";
          const verify = task.verify?.length ? `\n  - Verify: ${task.verify.join("; ")}` : "";
          const commit = task.commit ? `\n  - Commit contract: ${quoteMarkdown(task.commit)}` : "";
          const dependency = task.depends_on?.length
            ? `\n  - Depends on: ${task.depends_on.join(", ")}`
            : "";
          const parallel = task.parallel_group
            ? `\n  - Parallel group: ${task.parallel_group}`
            : "";
          const recovery = task.recovery_task_id
            ? `\n  - Recovery task: ${task.recovery_task_id}`
            : "";
          const attempts = task.max_attempts ? `\n  - Max attempts: ${task.max_attempts}` : "";
          return `- [ ] ${index + 1}.${taskIndex + 1} ${task.title}\n  - ID: ${task.id}\n  - Scope: ${quoteMarkdown(task.query)}${acceptance}${files}${verify}${commit}${dependency}${parallel}${attempts}${recovery}`;
        })
        .join("\n");
    })
    .join("\n");
...
<!-- Generated by snipara-companion. Tasks run inside managed workflow phases with bounded context and retries. -->
...
function buildLocalWorkflowPlan(
  ...
  return {
    ...
    steps: phases.map((phase) => ({
      ...
      ...(phase.dependsOn ? { depends_on: phase.dependsOn } : {}),
      ...(phase.parallelGroup ? { parallel_group: phase.parallelGroup } : {}),
      ...(phase.tasks && phase.tasks.length > 1
        ? {
            tasks: phase.tasks.map((task) => ({
              ...
              ...(task.dependsOn ? { depends_on: task.dependsOn } : {}),
              ...(task.parallelGroup ? { parallel_group: task.parallelGroup } : {}),
              ...(task.maxAttempts ? { max_attempts: task.maxAttempts } : {}),
              ...(task.recoveryTaskId ? { recovery_task_id: task.recoveryTaskId } : {}),
            })),
          }
        : {}),
