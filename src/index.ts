...
import { warnIfCompanionUpdateAvailable } from "./runtime/version-check";
...
} from "./commands/agent-context-evidence";
...
export {
  agentContextResolveCommand,
  agentContextValidateCommand,
  buildLocalAgentContextValidationReport,
  formatAgentContextResolution,
...
export {
  ...
  agentContextEvidenceStatusCommand,
  agentContextEvidenceTemplateCommand,
  readAgentContextEvidenceLedger,
} from "./commands/agent-context-evidence";
export {
  buildMemoryAudit,
  memoryAuditCommand,
...
program.showHelpAfterError("\nRun the command with --help to see valid options and examples.");
program.hook("preAction", () => {
  warnIfCompanionUpdateAvailable(readCliVersion());
});
...
function configureRealityCheckCommand(command: Command): Command {
  return command
    ...
    .option("--no-auto-context", "Disable bounded hosted/local context auto-linking")
    .option(
...
program
  ...
  .option("--target <target>", "Target agent or ADE (codex|claude-code|cursor|orca|kimi|custom)")
  .option("--changed-files <files...>", "Changed or relevant files")
  .option("--context <refs...>", "Context references, decisions, docs, or source facts")
  .option("--proof <proof...>", "Required proof gates or verification evidence")
  .option("--acceptance <criteria...>", "Acceptance criteria for the delegated work")
...
contextControl
  ...
  .option("--approve", "Acknowledge manual review when the local plan requires approval")
...
contextControl
  .command("hosted-diff")
  .description("Compare a local ProjectContext manifest with tenant-scoped hosted context")
...
contextControl
  ...
  .requiredOption("--plan <file>", "Plan JSON produced by context-control hosted-diff")
  .option("--approval <file>", "Resolved Decision Request artifact for mutating plans")
...
const agentContext = program
  ...
  .description("Compile role-scoped context and memory policy from a local manifest");

agentContext
  .command("validate")
  .description("Validate agent roles, local sources, budgets, and memory promotion rules")
  .option("--manifest <file>", "Agent Context manifest path", "snipara.agent-context.json")
  .option("--json", "Print raw JSON")
  .action(async (options) => {
    await agentContextValidateCommand({
      manifest: options.manifest,
      json: Boolean(options.json),
    });
  });
...
agentContext
  .command("resolve")
  .description("Resolve one agent's shared and role-specific context for a task")
  .requiredOption("--agent <agent>", "Agent alias or stable agent id")
  .requiredOption("--task <task>", "Current task")
  .option("--manifest <file>", "Agent Context manifest path", "snipara.agent-context.json")
  .option("--json", "Print raw JSON")
  .action(async (options) => {
    await agentContextResolveCommand({
      agent: options.agent,
      task: options.task,
      manifest: options.manifest,
      json: Boolean(options.json),
...
agentContextEvidence
  .command("template")
  .description("Create an editable evidence template linked to one resolved agent task")
  .requiredOption("--agent <agent>", "Agent alias or stable agent id")
  .requiredOption("--task <task>", "Representative task")
  .option("--manifest <file>", "Agent Context manifest path", "snipara.agent-context.json")
  .option("--output <file>", "Write the template to a JSON file")
  .option("--force", "Replace an existing output file")
  .option("--json", "Print raw JSON")
  .action(async (options) => {
    await agentContextEvidenceTemplateCommand({
      agent: options.agent,
      task: options.task,
      manifest: options.manifest,
      output: options.output,
...
agentContextEvidence
  .command("record")
  .description("Validate and append one immutable task receipt to the local evidence ledger")
  .requiredOption("--from <file>", "Completed AC-1 evidence JSON input")
  .option("--manifest <file>", "Agent Context manifest path", "snipara.agent-context.json")
  .option(
    "--ledger <file>",
    "Append-only evidence ledger path",
    ".snipara/agent-context/evidence.jsonl"
  )
  .option("--json", "Print raw JSON")
  .action(async (options) => {
    await agentContextEvidenceRecordCommand({
      from: options.from,
      manifest: options.manifest,
...
agentContextEvidence
  .command("status")
  .description("Aggregate the evidence ledger and evaluate the documented AC-1 exit gate")
  .option("--manifest <file>", "Agent Context manifest path", "snipara.agent-context.json")
  .option(
    "--ledger <file>",
    "Append-only evidence ledger path",
  ...
  .option("--enforce", "Exit non-zero until every AC-1 criterion passes")
  .option("--json", "Print raw JSON")
  .action(async (options) => {
    await agentContextEvidenceStatusCommand({
      manifest: options.manifest,
      ledger: options.ledger,
      enforce: Boolean(options.enforce),
      json: Boolean(options.json),
...
localWorkers
  ...
  .option("--api-key-env <env>", "Environment variable containing the provider API key")
  .option(
...
program
  .command("impact")
  .description("Run bounded code impact with automatic local, hosted, or hybrid source selection")
...
code
  ...
  .addCommand(
    new Command("local")
      ...
      .addCommand(
        new Command("callers")
          .description("List local AST callers or file importers with bounded transitive depth")
      ...
      .addCommand(
        new Command("neighbors")
          .description("Traverse the local AST and import graph around a symbol or file")
          .option("-q, --qualified-name <qualifiedName>", "Local symbol name or file::symbol")
          .option("--symbol-key <symbolKey>", "Local overlay symbol key")
          .option("-f, --file-path <filePath>", "File path to inspect")
  ...
  .addCommand(
    new Command("callers")
      .description("Find who calls a symbol with automatic local, hosted, or hybrid selection")
      .option("-q, --qualified-name <qualifiedName>", "Qualified symbol name")
  ...
  .addCommand(
    new Command("imports")
      .description("Find imports/importers with automatic local, hosted, or hybrid selection")
      .option("-q, --qualified-name <qualifiedName>", "Qualified symbol name")
      .option("--symbol-key <symbolKey>", "Stable graph or local overlay symbol key")
      .option("-f, --file-path <filePath>", "File path to inspect")
  ...
  .addCommand(
    new Command("neighbors")
      .description("Get a bounded symbol neighborhood with automatic graph-source selection")
      .option("-q, --qualified-name <qualifiedName>", "Qualified symbol name")
      .option("--symbol-key <symbolKey>", "Stable graph or local overlay symbol key")
      .option("-d, --depth <number>", "Traversal depth", "2")
      .option("--direction <direction>", "in|out|both", "both")
  ...
  .addCommand(
    new Command("shortest-path")
      .description("Find how two symbols connect with automatic graph-source selection")
  ...
  .addCommand(
    new Command("impact")
      .description("Run the primary agent-ready code impact gate with automatic source selection")
      .argument("[filePath]", "Source file to analyze")
      .option("-q, --qualified-name <qualifiedName>", "Qualified symbol name")
      .option("--symbol-key <symbolKey>", "Stable graph symbol key")
      .option("-f, --file-path <filePath>", "Source file to analyze")
...
Context vs Memory
  ...
  Context commands:
    ...
    agent-context   Compile organization, project, and role-specific retrieval policy
